...
{
	[Serializable]
	public class MFN_Z3K : AbstractMessage
	{
		...
		public MFN_Z3K(IModelClassFactory factory)
			: base(factory)
		{
			init(factory);
		}
		...
		private void init(IModelClassFactory factory)
		{
			try
			{
				...
				add(typeof(MFN_Z3K_Nofification), required: true, repeating: true);
			}
			catch (HL7Exception t)
			{
		...
		public MFN_Z3K_Nofification GetMF()
		{
			MFN_Z3K_Nofification mFN_Z3K_Nofification = null;
			try
			{
				return (MFN_Z3K_Nofification)GetStructure("Nofification");
			}
			catch (HL7Exception ex)
			{
				HapiLogFactory.GetHapiLog(GetType()).Error("Unexpected error accessing data - this is probably a bug in the source code generator.", ex);
		...
		public MFN_Z3K_Nofification GetMF(int rep)
		{
			return (MFN_Z3K_Nofification)GetStructure("Nofification", rep);
		}
	}
}
